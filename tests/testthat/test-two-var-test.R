...
test_that('output from var_test matches expected result', {
    ...
    expect_equivalent(unname(k$vars), c(14.90, 28.93))
    expect_equivalent(unname(k$avgs), c(16.62, 24.56))
    expect_equivalent(unname(k$sds), c(3.86, 5.38))
    expect_equivalent(unname(k$ses), c(0.91, 1.44))
    expect_equal(k$avg, 20.09)
    expect_equal(k$sd, 6.03)
    expect_equal(k$se, 1.07)
    expect_equal(k$len, 32)
    expect_equivalent(unname(k$lens), c(18, 14))
    expect_equal(unname(k$n1), 17)
    expect_equal(unname(k$n2), 13)
...
test_that('output from var_test matches expected result', {
    ...
    expect_equivalent(unname(k$vars), c(36.32, 3.19))
    expect_equivalent(unname(k$avgs), c(20.09, 17.85))
    expect_equivalent(unname(k$sds), c(6.03, 1.79))
    expect_equivalent(unname(k$ses), c(1.07, 0.32))
    expect_equal(k$avg, 18.97)
    expect_equal(k$sd, 4.55)
    expect_equal(k$se, 0.57)
    expect_equal(k$len, 64)
    expect_equivalent(unname(k$lens), c(32, 32))
